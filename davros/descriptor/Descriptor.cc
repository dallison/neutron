...
absl::Status Descriptor::SerializeToBuffer(davros::Buffer& buffer) const {
  ...
  for (auto& m : this->fields) {
    if (absl::Status status = m.SerializeToBuffer(buffer); !status.ok()) return status;
  }
  return absl::OkStatus();
}

absl::Status Descriptor::DeserializeFromBuffer(davros::Buffer& buffer) {
  ...
  for (int32_t i = 0; i < size; i++) {
    if (absl::Status status = this->fields[i].DeserializeFromBuffer(buffer); !status.ok()) return status;
  }
  return absl::OkStatus();
}
