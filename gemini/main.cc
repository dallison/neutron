...
ABSL_FLAG(std::string, runtime_path, "", "Path for gemini runtime");
...
void GenerateSerialization(const std::vector<std::filesystem::path> &files) {
  if (absl::GetFlag(FLAGS_all)) {
    std::shared_ptr<gemini::PackageScanner> scanner(
        new gemini::PackageScanner(files));
  ...
  std::shared_ptr<gemini::PackageScanner> scanner(
      new gemini::PackageScanner(roots));

  auto parse = [scanner](std::string file)
      -> absl::StatusOr<std::shared_ptr<gemini::Message>> {
    ...
    std::shared_ptr<gemini::Package> pkg = scanner->FindPackage(package_name);
    if (pkg == nullptr) {
      pkg = std::make_shared<gemini::Package>(nullptr, package_name);
  ...
  std::vector<std::shared_ptr<gemini::Message>> messages;
  for (auto &file : files) {
    absl::StatusOr<std::shared_ptr<gemini::Message>> msg = parse(file);
    if (!msg.ok()) {
